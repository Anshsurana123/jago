...
import com.example.jarvis.service.WakeWordService
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onRequestPermissionsResult(
        ...
        if (requestCode == 100) {
            if (grantResults.isNotEmpty() && grantResults.all { it == PackageManager.PERMISSION_GRANTED }) {
                ...
                // Permissions denied
                statusText.text = "Permissions required for Jarvis to work."
            }
        }
    }
    ...
    private fun requestDeviceAdminExemption() {
        ...
        val adminComponent = ComponentName(this, JarvisAdminReceiver::class.java)
        
        if (!dpm.isAdminActive(adminComponent)) {
            val intent = Intent(DevicePolicyManager.ACTION_ADD_DEVICE_ADMIN)
            intent.putExtra(DevicePolicyManager.EXTRA_DEVICE_ADMIN, adminComponent)
            intent.putExtra(DevicePolicyManager.EXTRA_ADD_EXPLANATION, "Jarvis needs this to lock your screen via voice.")
            startActivity(intent)
        } else {
