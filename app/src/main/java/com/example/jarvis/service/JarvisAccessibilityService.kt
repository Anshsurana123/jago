...
class JarvisAccessibilityService : AccessibilityService() {
    ...
    companion object {
        ...
        fun primeDirectShare() {
            ...
            Log.d("JarvisAccessibility", "Direct Share Auto-Send primed (Automation Active)")
        ...
        fun primeAutomation(app: String?, contact: String?) {
            ...
            isAutoSending = false
            Log.d("JarvisAccessibility", "Automation primed: App=$app, Contact=$contact")
        ...
        fun takeScreenshot(): Boolean {
            ...
            if (result) {
                Log.d("JarvisAccessibility", "Screenshot triggered")
            } else {
                Log.e("JarvisAccessibility", "Failed to trigger screenshot or service not running")
            }
            return result
        }
        ...
        fun clickShutter(): Boolean {
            ...
            for (node in nodes) {
                if (node.isClickable) {
                    ...
                    if (result) {
                        Log.d("JarvisAccessibility", "Shutter triggered")
                        return true
                    }
                }
            }
            Log.e("JarvisAccessibility", "Shutter button not found or not clickable")
            return false
        }

        fun clickFirstSpotifyResult(): Boolean {
            val root = instance?.rootInActiveWindow ?: return false
            Log.d("JarvisAccessibility", "Scanning for Spotify results...")
            ...
            if (nodes.isNotEmpty()) {
                ...
                for (node in nodes) {
                    if (node.isClickable) {
                         val result = node.performAction(AccessibilityNodeInfo.ACTION_CLICK)
                         if (result) {
                             Log.d("JarvisAccessibility", "Clicked Spotify node: ${node.viewIdResourceName} / ${node.className}")
                             return true
                         }
                    } else {
                        // Try parent
                         ...
                         while (parent != null) {
                             if (parent.isClickable) {
                                  ...
                                  if (result) {
                                      Log.d("JarvisAccessibility", "Clicked Spotify parent node")
                                      return true
                                  }
                                  break
                             }
            ...
            Log.e("JarvisAccessibility", "No suitable Spotify result found")
            return false
        }
        ...
        private fun waitForUI(timeoutMs: Long = 4000): AccessibilityNodeInfo? {
            ...
            while (System.currentTimeMillis() - start < timeoutMs) {
                ...
                if (root != null && root.childCount > 0) {
                    Log.d("JarvisAccessibility", "UI frame detected")
                    return root
                }
                try { Thread.sleep(120) } catch (e: InterruptedException) { e.printStackTrace() }
            }
            Log.d("JarvisAccessibility", "UI frame NOT detected (timeout)")
            return null
        }


        private fun selectContactViaSearch(contactName: String) {
            Log.d("JarvisAccessibility", "Starting Search Automation for: $contactName")
            ...
            val step1FindSearch = object : Runnable {
                ...
                override fun run() {
                    ...
                    if (root == null) {
                        ...
                        Log.e("JarvisAccessibility", "Failed to access root window for Search")
                    ...
                    if (searchNode != null) {
                        Log.d("JarvisAccessibility", "WhatsApp Search clicked")
                        ...
                        if (attempts < 5) {
                            ...
                            Log.e("JarvisAccessibility", "Search button not found")
                            Companion.isSearchingForContact = false
                        }
                    }
                }
        ...
        private fun enterContactName(contactName: String) {
             ...
             if (editText != null) {
                 ...
                 Log.d("JarvisAccessibility", "Contact name entered: $contactName")
                 ...
                 Log.e("JarvisAccessibility", "Search field (EditText) not found")
                 Companion.isSearchingForContact = false
             }
        }
             ...
             for (node in allNodes) {
                 if (node.isClickable && node.className != "android.widget.EditText" && node.className != "android.widget.ImageButton") {
                      ...
                      val text = node.text?.toString() ?: node.contentDescription?.toString() ?: ""
                       if (text.isNotEmpty()) {
                           Log.d("JarvisAccessibility", "First search result clicked: $text")
                           node.performAction(AccessibilityNodeInfo.ACTION_CLICK)
                           
                           // Success
                           Companion.targetContact = null
                           Companion.isSearchingForContact = false
                           com.example.jarvis.logic.JarvisTTS.speak("Ready to send")
                           return
                       }
                 }
             }
             
             Log.e("JarvisAccessibility", "No clickble search result found")
             Companion.isSearchingForContact = false
        }
    ...
    private fun startChooserPolling() {
        Log.d("JarvisAccessibility", "Starting chooser polling loop")
    ...
    private val chooserPollingRunnable = object : Runnable {
        override fun run() {
            ...
            if (Companion.chooserRetryCount >= Companion.MAX_CHOOSER_RETRIES) {
                 Log.e("JarvisAccessibility", "Chooser target not found after ${Companion.MAX_CHOOSER_RETRIES} attempts")
                 Companion.targetApp = null // Stop trying
                 return
            }

            Log.d("JarvisAccessibility", "Chooser scan attempt ${Companion.chooserRetryCount + 1}")
            ...
            if (root != null) {
                if (checkForAppInChooser(root, Companion.targetApp!!)) {
                 ...
                 Log.d("JarvisAccessibility", "Root window is null, waiting...")
            }
    ...
    private fun checkForAppInChooser(root: AccessibilityNodeInfo, appName: String): Boolean {
        ...
        if (matchedNode != null) {
             if (matchedNode.isClickable) {
                 Log.d("JarvisAccessibility", "Chooser target clicked: $appName")
                 ...
                 while (parent != null) {
                     if (parent.isClickable) {
                          Log.d("JarvisAccessibility", "Chooser target clicked (Parent): $appName")
    ...
    override fun onServiceConnected() {
        ...
        if (info != null) {
            ...
            serviceInfo = info
            Log.d("JarvisAccessibility", "Key filtering flag set explicitly")
        }
    }

    override fun onUnbind(intent: Intent?): Boolean {
        Log.d("JarvisAccessibility", "Service Unbound")
        instance = null
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (event?.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
            if (com.example.jarvis.logic.JarvisTTS.isSpeaking) {
                val packageName = event.packageName?.toString()
                if (packageName != null && packageName != "com.example.jarvis") {
                    Log.d("JarvisAccessibility", "Window changed to $packageName -> Stopping speech")
                    com.example.jarvis.logic.JarvisTTS.stopSpeaking()
                }
            }
        }
        ...
        if ((event?.eventType == AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED || event?.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED)) {
            ...
            if (Companion.pendingAutoSend && event.packageName == "com.whatsapp" && !Companion.isAutoSending) {
                Log.d("JarvisAccessibility", "WhatsApp UI detected. Starting Auto-Send Polling...")
    ...
    private fun startAutoSendPolling() {
        Log.d("JarvisAccessibility", "Auto-Send Polling Started")
        val handler = android.os.Handler(android.os.Looper.getMainLooper())
        ...
        val autoSendRunnable = object : Runnable {
            ...
            override fun run() {
                if (!Companion.pendingAutoSend) {
                     Log.d("JarvisAccessibility", "Auto-Send aborted or completed")
                     Companion.isAutoSending = false
                     return
                }

                if (attempts >= maxAttempts) {
                    Log.e("JarvisAccessibility", "Auto-Send timed out after 6 seconds")
                    
                    // Final Resort: Blind Gesture Click (User Requested)
                    Log.w("JarvisAccessibility", "Attempting Backup Gesture Click at (1121, 2522)")
                ...
                if (activeRoot != null) {
                    if (tryFindAndClickSend(activeRoot)) {
                     ...
                     Log.d("JarvisAccessibility", "Auto-Send: Root window null")
                }
    ...
    private fun performGestureClick(x: Float, y: Float) {
        ...
        val dispatched = dispatchGesture(gesture, object : android.accessibilityservice.AccessibilityService.GestureResultCallback() {
            override fun onCompleted(gestureDescription: android.accessibilityservice.GestureDescription?) {
                Log.d("JarvisAccessibility", "Gesture Click Completed at ($x, $y)")
            }

            override fun onCancelled(gestureDescription: android.accessibilityservice.GestureDescription?) {
                Log.e("JarvisAccessibility", "Gesture Click Cancelled")
            }
        }, null)
        
        Log.d("JarvisAccessibility", "dispatchGesture dispatched: $dispatched")
    }
        ...
        if (sendNodes != null && sendNodes.isNotEmpty()) {
            val sendButton = sendNodes[0]
             if (sendButton.isVisibleToUser && performRobustClick(sendButton)) {
                 Log.d("JarvisAccessibility", "Auto-Send SUCCESS via ID")
                 finishAutoSend()
                 return true
            }
        }
            ...
            for (node in textNodes) {
                ...
                if (node.className == "android.widget.ImageButton" && 
                    ...
                    if (node.isVisibleToUser && performRobustClick(node)) {
                         Log.d("JarvisAccessibility", "Auto-Send SUCCESS via Description")
                         finishAutoSend()
                         return true
                    }
                }
    ...
    private fun finishAutoSend() {
        ...
        if (Companion.automationActive) {
            Log.d("JarvisAccessibility", "Automation success. Returning to previous app in 500ms...")
            handler.postDelayed({
                returnToPreviousApp()
            }, 500)
    ...
    private fun returnToPreviousApp() {
        Log.d("JarvisAccessibility", "Performing Double Back Action to return to previous app")
        performGlobalAction(GLOBAL_ACTION_BACK)
        
        // WhatsApp often needs 2 back presses (Chat -> Main List -> Previous App)
        handler.postDelayed({
             Log.d("JarvisAccessibility", "Performing Second Back Action")
             performGlobalAction(GLOBAL_ACTION_BACK)
        }, 300)
    }
    ...
    override fun onKeyEvent(event: android.view.KeyEvent?): Boolean {
        if (event != null) {
            Log.d("JarvisAccessibility", "Key Event: ${event.keyCode}, Action: ${event.action}")
            if (event.keyCode == android.view.KeyEvent.KEYCODE_BACK && event.action == android.view.KeyEvent.ACTION_DOWN) {
                if (com.example.jarvis.logic.JarvisTTS.isSpeaking) {
                    Log.d("JarvisAccessibility", "Back button detected")
                    Log.d("JarvisAccessibility", "Stopping TTS from AccessibilityService")
                    com.example.jarvis.logic.JarvisTTS.stopSpeaking()
