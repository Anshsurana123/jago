...
import com.example.jarvis.service.JarvisAdminReceiver
...
class ActionExecutor(private val context: Context) {
    ...
    fun execute(command: Command) {
        when (command.type) {
            ...
            CommandType.OPEN_SCHEDULE -> {
                 ...
                 val intent = Intent(context, com.example.jarvis.ui.ScheduledTasksActivity::class.java).apply {
            ...
            CommandType.TAKE_SCREENSHOT -> {
                ...
                val success = JarvisAccessibilityService.takeScreenshot()
                if (success) {
            ...
            CommandType.CLICK_PHOTO -> {
                ...
                try {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed({
                        JarvisAccessibilityService.clickShutter()
                    }, 2500)
                } catch (e: Exception) {
            ...
            CommandType.SET_ALARM_CUSTOM -> {
                 ...
                 val calendar = java.util.Calendar.getInstance()
                     ...
                     val data = com.example.jarvis.logic.AlarmData(
                         triggerTimeMillis = calendar.timeInMillis,
                         message = command.messageBody ?: "Jarvis Alarm"
                     )
                     
                     com.example.jarvis.service.alarm.AlarmEngine.setAlarm(context, data)
            ...
            CommandType.PLAY_SPOTIFY -> {
                ...
                try {
                    ...
                    Handler(Looper.getMainLooper()).postDelayed({
                        ...
                        val success = JarvisAccessibilityService.clickFirstSpotifyResult()
                        if (!success) {
                             // Retry once more after a short delay
                             Handler(Looper.getMainLooper()).postDelayed({
                                 JarvisAccessibilityService.clickFirstSpotifyResult()
                             }, 1000)
                        }
    ...
    private fun lockDevice() {
        ...
        val adminComponent = ComponentName(context, JarvisAdminReceiver::class.java)
        
        if (dpm.isAdminActive(adminComponent)) {
            Log.d("JarvisAdmin", "Executing lock command")
            speak("Locking the device")
            dpm.lockNow()
        } else {
            Log.d("JarvisAdmin", "Admin not active")
    ...
    private fun sendDirectWhatsAppMessage(phoneNumber: String, message: String) {
        ...
        try {
            ...
            com.example.jarvis.service.JarvisAccessibilityService.primeDirectShare()
        ...
        if (photoUri != null) {
            ...
            if (!directShareSuccess) {
                ...
                if (appName != null || contactName != null) {
                    com.example.jarvis.service.JarvisAccessibilityService.primeAutomation(appName, contactName)
    ...
    private fun setAlarm(hour: Int, minute: Int, message: String? = "Jarvis Alarm") {
        try {
